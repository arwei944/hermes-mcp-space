/**
 * RulesEntryWidget.js
 * 规则入口卡片 - 显示规则总数，点击跳转到知识库页面
 */
const RulesEntryWidget = (() => {
    'use strict';
    ...
    async function mount(container, props) {
        ...
        let data;
        try {
            data = await DataService.fetch('rules');
        } catch (e) {
            data = null;
        }
        ...
        container.innerHTML = `
            <div class="ws-widget" style="cursor:pointer; text-align:center; padding:20px;">
                ...
                <div style="font-size:var(--text-lg); font-weight:600; color:var(--text-primary);">规则</div>
                <div style="font-size:var(--text-2xl); font-weight:700; color:var(--accent); margin-top:8px;">${count}</div>
                <div style="font-size:var(--text-xs); color:var(--text-tertiary); margin-top:4px;">条规则</div>
            </div>`;
        ...
        return {
            ...
            async refresh() {
                let newData;
                try {
                    newData = await DataService.fetch('rules');
                } catch (e) {
                    newData = null;
                }
