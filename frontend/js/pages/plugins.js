...
const PluginsPage = (() => {
    ...
    function buildPage() {
        ...
        const typeCounts = { all: _market.length, tool: _market.filter(p => p.type === 'tool').length, skill: _market.filter(p => p.type === 'skill').length, memory: _market.filter(p => p.type === 'memory').length };
    ...
    function switchTab(tab) { _tab = tab; document.getElementById('contentBody').innerHTML = buildPage(); bindEvents(); }
    ...
    function bindEvents() {
        ...
        if (search) search.addEventListener('input', Components.debounce(e => { _keyword = e.target.value; document.getElementById('contentBody').innerHTML = buildPage(); bindEvents(); }, 300));
        const typeSelect = document.getElementById('pluginType');
        if (typeSelect) typeSelect.addEventListener('change', e => { _filterType = e.target.value; document.getElementById('contentBody').innerHTML = buildPage(); bindEvents(); });
        const catSelect = document.getElementById('pluginCategory');
        if (catSelect) catSelect.addEventListener('change', e => { _filterCategory = e.target.value; document.getElementById('contentBody').innerHTML = buildPage(); bindEvents(); });
    }
