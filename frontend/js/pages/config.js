...
const ConfigPage = (() => {
    ...
    function getMockConfig() {
        return {
            ...
            memoryEnabled: true, autoSaveMemory: true, maxContextLength: 128000,
            mcpEnabled: true, mcpPort: 3000, logLevel: 'info', debugMode: false,
        };
    }
                ...
                ${Components.formGroup('启用的工具集', `
                    <div style="display:flex;flex-wrap:wrap;gap:12px;margin-top:4px">
                        ${['filesystem', 'system', 'web', 'creative', 'memory', 'skills', 'mcp'].map(ts => `
                            ${Components.formSwitch('toolset_' + ts, ts, (c.enabledToolsets || []).includes(ts))}
                        `).join('')}
                    </div>
                `)}
            `)}
    ...
    function collectFormData() {
        ...
        ['filesystem', 'system', 'web', 'creative', 'memory', 'skills', 'mcp'].forEach(ts => {
            ...
            if (checkbox && checkbox.checked) config.enabledToolsets.push(ts);
        });
        return config;
    }
    ...
    return { render, saveConfig, resetConfig };
})();
