...
class DataIntegrityChecker:
    ...
    def run_full_check(self, auto_fix: bool = True) -> Dict[str, Any]:
        ...
        if status == "ok":
            ...
            logger.warning(
                f"Data integrity check found {total_errors} issues, "
                f"auto-fixed {total_fixes}"
            )
    ...
    def _fix_dual_write_consistency(self):
        ...
        try:
            ...
            session_diff = abs(db_session_count - json_session_count)
            message_diff = abs(db_message_count - json_message_count)
            if session_diff == 0 and message_diff == 0:
                self._add_check("dual_write:consistency", True,
                    f"SQLite({db_session_count}s/{db_message_count}m) == JSON({json_session_count}s/{json_message_count}m)")
            else:
                self._add_check("dual_write:consistency", False,
                    f"数据不一致: SQLite({db_session_count}s/{db_message_count}m) vs JSON({json_session_count}s/{json_message_count}m)")
